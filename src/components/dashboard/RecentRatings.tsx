      ...
      {ratings.length > 0 ? (
        <div className="flex flex-col gap-2.5 max-h-[420px] overflow-y-auto pr-2">
          {ratings.map((rating, i) => (
            <div
              key={rating.id}
              className="flex items-center gap-4 p-4 rounded-xl bg-white/[0.02] border border-white/[0.04] hover:bg-white/[0.04] hover:border-white/[0.08] transition-all duration-300 animate-fade-in"
              style={{ animationDelay: `${i * 60}ms` }}
            >
              <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-primary/30 to-secondary/30 flex items-center justify-center text-primary-light font-bold text-sm shrink-0 border border-primary/10">
