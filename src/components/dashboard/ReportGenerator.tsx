...
<body>
  ...
  <div class="section">
    <h2>&#9733; Top Fiverr Profiles</h2>
    <table>
      <thead><tr><th>Rank</th><th>Profile</th><th>5&#9733; Ratings</th></tr></thead>
      <tbody>
        ${(() => {
          const profileMap = new Map<string, Rating[]>();
          allRatings.forEach(r => {
            if (!r.profile_name) return;
            const key = r.profile_name.toLowerCase();
            if (!profileMap.has(key)) profileMap.set(key, []);
            profileMap.get(key)!.push(r);
          });
          return Array.from(profileMap.entries())
            .map(([name, pRatings]) => ({ name, count: countFiveStarOrders(pRatings) }))
