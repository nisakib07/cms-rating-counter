...
export default function TeamChart({ teams }: TeamChartProps) {
  ...
  return (
    <div className="glass rounded-2xl p-7 animate-fade-in delay-400 relative overflow-hidden h-full">
        ...
        {data.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 5, right: 10, left: -15, bottom: 20 }}>
              <XAxis dataKey="name" tick={{ fill: '#64748b', fontSize: 11, angle: -25, textAnchor: 'end' }} axisLine={false} tickLine={false} height={40} />
              <YAxis tick={{ fill: '#64748b', fontSize: 11 }} axisLine={false} tickLine={false} allowDecimals={false} />
              <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(255,255,255,0.02)' }} />
              <Bar dataKey="rating_count" radius={[8, 8, 0, 0]} maxBarSize={36}>
                {data.map((entry, index) => (
                  <Cell key={index} fill={entry.color || (entry.service_line === 'CMS Hub' ? '#10b981' : '#3b82f6')} fillOpacity={0.85} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
