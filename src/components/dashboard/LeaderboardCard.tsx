    ...
    <div className="glass rounded-2xl p-7 animate-fade-in delay-300 relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 relative z-10">
        ...
        <div className="relative bg-gradient-to-br from-warning/10 via-warning/5 to-transparent rounded-2xl p-5 border border-warning/10 overflow-hidden group hover:border-warning/20 transition-all duration-300 card-hover">
          ...
          {topMember ? (
            <Link href={`/members/${topMember.id}`} className="flex items-center gap-4 group/link">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-warning to-orange-500 flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-warning/25 overflow-hidden">
                {topMember.profile_image ? (
                  <img src={toDriveDirectUrl(topMember.profile_image)} alt={topMember.name} className="w-full h-full object-cover" onError={e => { e.currentTarget.style.display = 'none'; e.currentTarget.parentElement!.textContent = topMember.name.charAt(0); }} />
                ) : topMember.name.charAt(0)}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-bold text-text-primary text-lg truncate group-hover/link:text-primary-light transition-colors">{topMember.name}</div>
        ...
        <div className="relative bg-gradient-to-br from-primary/10 via-primary/5 to-transparent rounded-2xl p-5 border border-primary/10 overflow-hidden group hover:border-primary/20 transition-all duration-300 card-hover">
          ...
          {topTeam ? (
            <Link href={`/teams/${topTeam.id}`} className="flex items-center gap-4 group/link">
